...
pub fn spawn_rotation_thruster_cone(
    ...
    let (n_mul, life_mul) = match entities_quantity {
        EntitiesQuantity::Some => (8, 1.0),
        EntitiesQuantity::ALot => (12, 1.3),
        EntitiesQuantity::TooMuch => (20, 1.8),
    };
    ...
    for _ in 0..(n * n_mul) {
        ...
        commands.spawn((
            ...
            Particle {
                lifetime: rng.gen::<f32>().abs().min(1.0).powf(4.0) * 0.25 * life_mul + 0.05,
        ...
        for (transform, velocity, thruster) in ships.iter() {
            if abs_cam_area.contains(transform.translation.xy()) {
                ...
                let (n_mul, life_mul) = match game_settings.entities_quantity {
                    EntitiesQuantity::Some => (30.0, 1.0),
                    EntitiesQuantity::ALot => (50.0, 1.3),
                    EntitiesQuantity::TooMuch => (100.0, 1.8),
                };
                ...
                for _ in 0..n {
                    ...
                    commands.spawn((
                        ...
                        Particle {
                            lifetime: rng.gen::<f32>().abs().powf(2.0) * 0.25 * life_mul + 0.1,
                            spawned_at: time.elapsed_seconds(),
                            // velocity: vel,
                            kind: ParticleKind::Combustion {
                                init_radius: 1.0,
